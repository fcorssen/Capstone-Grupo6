...
# -------------- Agregar Recorrido ----------------
points = random.choices(coordinate_eccomerce, k=30)
sorted(points , key=lambda k: [k[1], k[0]])
points.insert(0, random.choice(coordinate_driver))
points.append(random.choice(coordinate_driver))
...
points_delivery = random.choices(locations[:amountDays[0]], k=30)
...
for i in range(len(points_delivery)):
     folium.Marker([points_delivery[i][0], points_delivery[i][1]], icon=DivIcon(
        icon_size=(150,36),
        icon_anchor=(7,20),
        html=f'<div style="font-size: 18pt; color : Gray">{i + 32}</div>',
        )).add_to(m)
